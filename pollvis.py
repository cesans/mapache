...
class PollVis:
    ...
    def __scatter(self, x, y, ax, partyname, single=False):
        if single:
            ...
            ax.scatter(x,y, c=np.append(self.parties[partyname].color,0.6), edgecolors='none', s=40, label=u'Observations')
    ...
    def __gp(self, x, y, ax, partyname):
        ...
        ax.fill(np.concatenate([x_dense, x_dense[::-1]]),
                ...
                color=np.append(self.parties[partyname].color,0.1), fc='b', ec='None', label='95% confidence interval')
